...
export default function V338() {
  ...
  return (
    <div className="relative h-screen font-['Ubuntu',sans-serif] bg-white overflow-hidden">
      <style jsx>{`
        ...
        @keyframes animationFramesMobile {
          0% { transform: translateX(0) rotateY(180deg); z-index: 1; }
          50% { transform: translateX(-150%) rotateY(0deg) rotateZ(15deg) scale(0.7, 0.7); z-index: -10; }
          100% { transform: translateX(0) rotateY(0deg) scale(0.9, 0.9); z-index: -10; }
        }
        ...
        @media (max-width: 768px) {
          .hide-animation {
            animation: animationFramesMobile linear 0.55s;
            animation-iteration-count: 1;
          }
        }
      `}</style>
      ...
      {/* Card Counter */}
      <div className="relative top-4  text-base md:text-lg text-center font-semibold text-gray-700 z-50">
        Card {currentCardIndex + 1} of {cardContents.length}
      </div>

      {/* Deck Container - Responsive positioning */}
      <div className="absolute bottom-[40%] left-1/2 -translate-x-1/2 md:bottom-[30%] md:left-auto md:right-[30%] md:translate-x-0 p-2.5 transform-gpu">
        {/* Back cards */}
        <div
          className="w-[140px] h-[192.5px] md:w-[200px] md:h-[275px] rounded-lg mb-1 shadow-[0px_3px_5px_rgba(0,0,0,0.5)] absolute top-0 border border-[#726A53] bg-[#F8F5ED] overflow-hidden p-2 box-border"
          style={{
        ...
        >
          {/* Green Card */}
          <div
            className={`w-[140px] h-[192.5px] md:w-[200px] md:h-[275px] rounded-lg mb-1 absolute top-0 border border-[#726A53] bg-red-500 overflow-hidden p-2 box-border z-[1] ${showSpecialCard ? 'block' : 'hidden'}`}
            style={{
              backgroundImage: "url('https://s-media-cache-ak0.pinimg.com/236x/c1/59/b4/c159b4738dae9c9d8d6417228024de8d.jpg')",
              backgroundSize: '102.5% 101.5%',
              backgroundPosition: 'center',
          ...
          {/* Flipper */}
          <div
            className={`relative w-[140px] h-[192.5px] md:w-[200px] md:h-[275px] transition-transform duration-[600ms] ${isHiding ? 'hide-animation -z-[1]' : ''}`}
            style={{
          ...
          >
            {/* Front */}
            <div
              className="absolute top-0 left-0 w-[140px] h-[192.5px] md:w-[200px] md:h-[275px]"
              style={{
            ...
            >
              <div
                className={`w-[140px] h-[192.5px] md:w-[200px] md:h-[275px] rounded-lg shadow-[0px_3px_5px_rgba(0,0,0,0.5)] border border-[#726A53] bg-[#F8F5ED] overflow-hidden p-2 box-border transition-transform duration-[600ms]`}
                style={{
                  WebkitFontSmoothing: 'antialiased',
                  transformOrigin: 'right top',
                  transform: isFlipped ? 'scale(2, 2)' : 'scale(1, 1)',
                  backgroundImage: "url('https://s-media-cache-ak0.pinimg.com/236x/c1/59/b4/c159b4738dae9c9d8d6417228024de8d.jpg')",
                  backgroundSize: '102.5% 101.5%',
                  backgroundPosition: 'center',
                  boxShadow: 'none',
            ...
            >
              <div
                className={`w-[280px] h-[385px] md:w-[500px] md:h-[687.5px] rounded-lg border border-[#726A53] bg-[#F8F5ED] overflow-hidden overflow-y-auto p-3 md:p-4 box-border text-sm md:text-lg transition-transform duration-[600ms]`}
                style={{
              ...
              >
                <h4 className="font-bold text-base md:text-xl mb-2 md:mb-3">{currentCard.title}</h4>
                {currentCard.content.map((paragraph, index) => (
                  <p key={index} className="p-0 mt-0 mb-2 md:mb-3 text-left leading-relaxed">
                    {paragraph}
                  </p>
                ))}
              </div>
