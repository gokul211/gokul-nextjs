...
export default function SpreadFxGallery() {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e) => {
      ...
      cubes.forEach(cube => {
        if (!cube.matches(':hover') && cube.getAttribute('data-index') !== activeCube) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center overflow-hidden bg-gradient-to-br from-[#111] to-[#222] text-white font-['Segoe_UI',_Tahoma,_Geneva,_Verdana,_sans-serif] relative">
      ...
      {/* Title */}
      {/* <div className="text-[2em] font-mono font-small-caps tracking-[0.8em] my-12 text-center relative px-8 py-4 bg-black/30 rounded-lg shadow-[0_0_25px_rgba(255,255,255,0.15)] z-10 backdrop-blur-sm">
        SPREAD FX
        <div className="absolute bottom-[-12px] left-1/2 transform -translate-x-1/2 w-32 h-1 bg-gradient-to-r from-transparent via-coral-500 to-transparent"></div>
      </div> */}

      {/* Circular Gallery Container */}
      <div 
        ref={galleryRef}
        className="relative top-20 w-full h-[75vh] max-w-[1400px] flex items-center justify-center mb-8 z-10"
      >
        {/* Central Cube - Larger and more prominent */}
        <ul
          data-index="center"
        ...
        {cubeData.map((cube, index) => {
          const position = getCircularPosition(index, cubeData.length, 280);
          ...
          return (
            <ul
              ...
              } ${activeCube !== null && activeCube !== index ? 'inactive' : ''} ${
                isMounted ? 'opacity-100' : 'opacity-0'
              }`}
              style={{
                transform: 'rotateX(0deg) rotateY(0deg) rotateZ(0deg)',
                ...
                marginLeft: `-${cube.width / 2}px`,
                marginTop: `-${cube.height / 2}px`
              }}
        ...
        {/* Circular Guide Line (Visual Only) */}
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[560px] h-[560px] border border-white/10 rounded-full"></div>
      ...
      {/* Info Panel */}
      <div className="info-panel fixed bottom-8 left-1/2 transform -translate-x-1/2 bg-black/60 px-8 py-4 rounded-2xl text-center text-base backdrop-blur-lg border border-white/15 opacity-0 transition-all duration-500 pointer-events-none z-20 shadow-2xl">
        <div className="flex items-center justify-center gap-3">
          <div className="w-2 h-2 bg-coral-500 rounded-full animate-pulse"></div>
          <span>Click on any cube to expand • Hover to see the spread effect</span>
          <div className="w-2 h-2 bg-coral-500 rounded-full animate-pulse"></div>
        </div>
      </div>
      ...
      <style jsx global>{`
        ...
        /* Active (clicked) state - reduced scale */
        .cube.active {
