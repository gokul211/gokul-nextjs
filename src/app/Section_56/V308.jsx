...
export default function V308() {
  ...
  useEffect(() => {
    ...
    // ---- Begin adapted simulation script (client-side) ----
    'use strict';
    ...
    // small embedded dithering image (1x1 white fallback). You can replace with a noisier base64 PNG if desired.
    let ditheringTexture = createTextureAsync('data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR4nGNgYAAAAAMAASsJTYQAAAAASUVORK5CYII=');
    ...
    // --- Overlay controls (React buttons will call these via window for convenience)
    ...
    // bind capture function to window for the React button to call
    window.captureSimulationPNG = function() {
      try {
        captureScreenshot();
      } catch (err) {
        console.warn('Capture failed — simulation may not be initialized yet.', err);
      }
    }

    // ---- End adapted simulation script ----

    // cleanup: remove global functions on unmount
    return () => {
      try {
        delete window.hideOverlay;
        delete window.showOverlay;
        delete window.captureSimulationPNG;
      } catch (e) {}
    };
  }, []);

  // Buttons call functions defined on window by the script
  function onHide() {
    setHidden(true);
  ...
  function onCapture() {
    if (window.captureSimulationPNG) window.captureSimulationPNG();
    else console.warn('Capture not initialized yet.');
  }
  ...
  return (
    <div className="w-full h-full relative bg-black">
      ...
      {/* <div
        ...
        role="dialog"
        aria-label="Info"
        className={`absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10 bg-black bg-opacity-60 text-white p-6 rounded-xl text-center min-w-[280px] max-w-[90%] shadow-xl backdrop-blur-sm ${hidden ? 'hidden' : ''}`}
      >
        <button
          onClick={onHide}
          title="Close overlay"
          className="absolute -right-2 -top-2 bg-white/12 text-white w-7 h-7 rounded-full flex items-center justify-center font-semibold shadow"
        >
          ✕
        </button>
        <h1 className="m-0 mb-2 text-lg font-semibold">Fluid Simulation</h1>
        <p className="m-0 mb-4 text-sm text-slate-200 leading-5">
          Move your mouse or touch the screen to interact. Press <strong>P</strong> to pause, <strong>Space</strong> to add random splats.
        </p>
        <div className="flex gap-2 justify-center">
          <button onClick={onCapture} className="px-3 py-2 rounded-lg text-sm bg-gradient-to-r from-[#ff6a88] to-[#ff9a6a] text-white">
            Capture PNG
          </button>
          <button onClick={onHide} className="px-3 py-2 rounded-lg text-sm bg-white/6 text-white">
            Hide
          </button>
        </div>
      </div> */}

      {/* If hidden show a small restore button */}
      {hidden && (
        <button
          onClick={onShow}
          className="absolute left-4 top-4 z-20 bg-white/8 text-white px-3 py-2 rounded-md"
        >
          Show overlay
        </button>
      )}
