...
export default function V254() {
  ...
  useEffect(() => {
    ...
    // Modified controller click handler to prioritize lightbox
    controller.addEventListener("click", (e) => {
      e.stopPropagation();
      // If an overlay is active, close it
      if (overlayMesh) {
        ...
        if (original) {
          ...
          gsap.to(qt, {
            ...
            ease: "power2.out",
            onUpdate: () => {
              overlayMesh.quaternion.slerp(endQ, qt.t);
            },
            onComplete: () => {
      ...
      // If no overlay, try to open lightbox for the front item
      const front = getFrontItem();
      if (front) {
        openLightbox(front.mesh.userData.index);
        return; // Prevent further processing if lightbox is opened
      }

      // Original overlay zoom logic (if no front item or lightbox not opened)
      if (!front) return; // This line might be redundant if the above `if (front)` handles it
    ...
    function onCanvasClick(ev) {
      // Only trigger if not dragging and not interacting with joystick
      if (Math.abs(vel.x) > 0.02 || Math.abs(vel.y) > 0.02 || dragging || jsDragging) return;
    ...
    <div style={{ height: "100vh", width: "100vw", position: "relative", overflow: "auto" }}>
      ...
      <style>{`
        ...
        @media (max-width: 768px) {
          :root {
            --radius: 200; /* Smaller radius for mobile */
            --card-w: 80;  /* Smaller card width */
            --card-h: 60;  /* Smaller card height */
            --sphereSize: 80px; /* Smaller controller size */
            --textSpeed: 8s; /* Faster text scroll for smaller screens */
            --textGap: 10px;
          }

          .controller {
            bottom: 20px; /* Position controller at the bottom */
            top: auto;
            left: 50%;
          ...
          .joystick {
            top: 20px; /* Position joystick at the top */
            bottom: auto;
            left: 50%;
            transform: translateX(-50%);
          }
