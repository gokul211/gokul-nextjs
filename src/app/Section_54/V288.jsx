...
export default function V288() {
  ...
  return (
    <section className="max-w-[1100px] mx-auto px-5 py-10 bg-[#f8f8f8]">
      <h2 className="text-center text-[1.6rem] md:text-[2rem] font-bold mb-6 md:mb-10 leading-tight text-[#333] px-2">
        Discover how Hautech.ai has helped<br className="hidden md:block" />
        <span className="md:inline"> its customers streamline content production</span>
      </h2>

      {/* Desktop Layout */}
      <div className="hidden md:flex gap-8 items-center justify-center relative">
        {/* Image Column */}
      ...
      <div className="md:hidden">
        ...
        <div className="relative w-full mb-6">
          <div className="absolute top-[10px] left-[-20px] w-[calc(100%-40px)] h-[calc(100%-10px)] bg-[#e9e9e9] rounded-xl z-0 shadow-[0_6px_14px_rgba(0,0,0,0.05)] rotate-[0.4deg]" />
          <div className="absolute top-[6px] left-[-12px] w-[calc(100%-24px)] h-[calc(100%-6px)] bg-[#f3f3f3] rounded-xl z-0 shadow-[0_4px_10px_rgba(0,0,0,0.04)] rotate-[0.15deg]" />
          
          <div className="relative rounded-xl overflow-hidden h-[280px] z-[3]">
            {testimonials.map((t, i) => (
              <div
                key={i}
              ...
              >
                <img
                  src={t.desktopImg}
                  alt={t.title}
                  className="w-full h-full object-contain rounded-xl shadow-[0_10px_25px_rgba(0,0,0,0.08)]"
                />
              </div>
            ))}
          </div>
        ...
        <div className="flex justify-center gap-3 mb-5">
          <button
            onClick={() => { changeSlide(-1); resetAutoplay(); }}
            className="w-10 h-10 rounded-full bg-white flex items-center justify-center shadow-[0_4px_10px_rgba(0,0,0,0.06)] cursor-pointer transition-all duration-[180ms] active:scale-95"
            aria-label="Previous"
          >
        ...
        <div className="relative w-full overflow-hidden px-2">
          {testimonials.map((t, i) => (
            <div
              key={i}
              className={`${i === currentSlide ? 'relative' : 'absolute'} top-0 left-0 w-full px-2 transition-all duration-[600ms] ease-in-out ${
                i === currentSlide ? 'opacity-100 translate-x-0 z-[2]' : 'opacity-0 translate-x-5'
              }`}
            >
              <div className="flex gap-2 mb-3 flex-wrap">
                {t.tags.map((tag, idx) => (
                  <div key={idx} className="bg-[#f0f0f0] px-2.5 py-1.5 rounded-[18px] text-[0.75rem] font-medium">
                    {tag}
                  </div>
                ))}
              </div>
              <h3 className="text-[1.2rem] text-left font-bold mb-3 leading-tight">{t.title}</h3>
              <p className="text-left mb-4 text-[#555] text-[0.9rem] leading-relaxed">
                {t.description}
              </p>
              <a
                href={t.link}
                className="block text-left text-black font-semibold no-underline border-black pb-0.5 transition-all duration-[180ms] text-[0.85rem]"
              >
                {t.link.replace(/^https?:\/\//, '').substring(0, 25)}
                {t.link.length > 25 ? '...' : ''}
              </a>
            </div>
          ))}
      ...
      {/* Slide Indicators */}
      <div className="flex justify-center gap-2 mt-6 md:mt-5">
        {testimonials.map((_, i) => (
          <button
            key={i}
            onClick={() => goToSlide(i)}
            className={`w-2 h-2 md:w-2.5 md:h-2.5 rounded-full cursor-pointer transition-all duration-[300ms] ${
